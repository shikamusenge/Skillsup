...
if (mysqli_num_rows($sql)==0) {
	echo "You have not Suggested any group";
}
else
{
	echo "<ul>";
	while ($row=mysqli_fetch_array($sql)) {
		...
		echo "<li>".$row['GroupName']."<a href='app.php?GroupId=".$row['GroupId']."' class='btn btn-sm btn-primary my-5'>join</a></li>";
	}
	echo"</ul>";
}}

/// display groups convensation
else{  $sq= mysqli_query($con,"SELECT * FROM groups where GroupId='$_GET[GroupId]'");
...
<?php
  ...
  $sql = mysqli_query($con,"SELECT * FROM `post` INNER JOIN `attachments` inner join users WHERE post.DocId=attachments.attch_id and  users.UserId=post.DocumentName AND status='group' and post.UserId='$_GET[GroupId]' ORDER BY post.DocId Desc") or die("erro");
if (mysqli_num_rows($sql)==0) {
	echo "<div class='d-flex col-12' style='justfy-content:space-around'>Goup have not added any convernsation</div>";
}
else
{
	while ($row=mysqli_fetch_array($sql)) {
        ...
        <form action="app.php" method="post" style="position:relative;">
        <input type="hidden" name="PostId" value="'.$row['PostId'].'"><input type="hidden" name="GroupId" value="'.$_GET['GroupId'].'">
